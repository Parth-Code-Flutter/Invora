...
void main() {
  testWidgets('overview hero amount stays clear of the sparkline', (
    tester,
  ) async {
    ...
    expect(find.text('Invoiced this month'), findsOneWidget);

    final amountRect = tester.getRect(find.text('₹2,308,000'));
    final chartRect = tester.getRect(
      find.byKey(const ValueKey('dashboard-cash-flow')),
    );
    expect(amountRect.overlaps(chartRect), isFalse);
    expect(amountRect.bottom, lessThanOrEqualTo(chartRect.top));
